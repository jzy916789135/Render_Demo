...
int main()
{
	...
	Shader ourShder("shadervs_lo_sys.txt", "shaderfs_lo_sys.txt");
	...
	ourShder.setFloat("Mixvalue", 0.3f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ourShder.use();
		unsigned int modelLocation = glGetUniformLocation(ourShder.ID, "model");
		unsigned int viewLocation = glGetUniformLocation(ourShder.ID, "view");
		unsigned int projectionLocation = glGetUniformLocation(ourShder.ID, "projection");
		...
		for (int i = 0; i < 10; i++)
		{
			...
			if(i<3)
				model = glm::rotate(model, float(i+1) * float(glfwGetTime()), glm::vec3(1.0f, 0.3f, 0.5f));
			else
