...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(800, 600, "Depth_test", NULL, NULL);
	if (window == NULL)
	{
    ...
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);//源alpha分量与 1-alpha分量
    glEnable(GL_CULL_FACE);//面剔除
    glCullFace(GL_FRONT);//选择剔除正向面
    glFrontFace(GL_CW);//正向面为顺序    上述效果等同于默认效果

	Shader shader("1.1.depth_testingvs.txt","1.1.depth_testingfs.txt");
    Shader shaderSingleColor("1.1.depth_testingvs.txt", "Stencilfs.txt");
    float cubeVertices[] = {
        // Back face
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        */
        ...
        glDisable(GL_DEPTH_TEST);//让边框不会被地板覆盖
        shaderSingleColor.use();
        float scale = 1.05;
        // cubes
        glBindVertexArray(cubeVAO);
