...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	lastX = xpos;
	lastY = ypos;
	camera.ProcessMouseMovement(xoffest, yoffest);
}
...
int main()
{
	...
	Shader shader("1.1.depth_testingvs.txt","1.1.depth_testingfs.txt");
	Shader screenShader("framebuffer_vs.txt","framebuffer_fs.txt");
	...
	unsigned int floorTextture = loadTexture("container.jpg");
	...
	//create a color attachment texture 创建纹理附件
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//render
		//--------
		//bind FrameBuffer and draw scene as we normally would to color texture
		...
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(2.0f, 0.001f, .0f));
		shader.setMat4("model", model);
		glDrawArrays(GL_TRIANGLES, 0, 36);
		//floor
		glBindVertexArray(planeVAO);
		glBindTexture(GL_TEXTURE_2D, floorTextture);
		model = glm::mat4(1.0f);
		shader.setMat4("model", model);
