...
int main()
{
	...
	Shader ourShader("shadervs_lo_sys.txt","shaderfs_lo_sys.txt");
	float vertices[] = {
	...
	ourShader.setFloat("Maxvalue", 0.5f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i =1 ;i < 10; i++)
		{
			...
			model = glm::rotate(model, float(i + 1) * float(glfwGetTime()), glm::vec3(1.0f, 0.5f, 0.3f));
